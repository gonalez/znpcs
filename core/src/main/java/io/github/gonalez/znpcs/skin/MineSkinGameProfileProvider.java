...
public class MineSkinGameProfileProvider extends HttpGameProfileProvider {
    ...
    return super.prepareRequest(uri, skin)
        ...
        .POST(
            BodyPublishers.ofString(
                GSON.toJson(MineSkinGenerateRequest.newBuilder().setUrl(skin).build())));
  }
  ...
  protected GameProfile provideGameProfile(String skin, JsonElement value) {
    JsonObject skinObject = value.getAsJsonObject().getAsJsonObject("skin");

    JsonObject textureData = skinObject.getAsJsonObject("texture").getAsJsonObject("data");
